        ...
        <div class="max-w-3xl bg-slate-800 rounded-md shadow overflow-hidden">
            <form @submit.prevent="store">
                <div class="flex flex-wrap -mb-8 -mr-6 p-8">
                    <text-input v-model="form.name" maxlength="30" :error="form.errors.name" class="pb-8 pr-6 w-full lg:w-1/2" aria-label="Recipe Name" label="Name" />
                    <text-input v-model="form.description" maxlength="30" :error="form.errors.description" class="pb-8 pr-6 w-full lg:w-1/2" aria-label="Recipe Description" label="Description" />
                    ...
                    <select-input v-model="form.type" :error="form.errors.type" class="pb-8 pr-6 w-full lg:w-1/2" aria-label="Recipe Type" label="Type">
                        <option v-for="tp in type" :key="tp.id" :value="tp.id">{{ tp.type }}</option>
                    </select-input>
                    <file-input-photo v-model="form.photo" :error="form.errors.photo" class="pb-8 pr-6 w-full lg:w-1/2" type="file" accept="image/*" aria-label="Recipe Photo" label="Photo" id="photo" />
                    <select-input multiple v-model="form.ingredients" :error="form.errors.ingredients" class="pb-8 pr-6 w-full lg:w-1/2" aria-label="Recipe Ingredients" label="Ingredients" name="ingredients[]">
                ...
                <div class="flex items-center justify-end px-8 py-4 bg-slate-800 border-t border-gray-100 font-lora">
                    <loading-button :loading="form.processing" class="px-4 py-2 ml-auto rounded text-black bg-yellow-400 hover:bg-yellow-300 transition duration-500" aria-label="Create Recipe" type="submit">Create Recipe</loading-button>
                </div>
            </form>
        </div>
    </div>
